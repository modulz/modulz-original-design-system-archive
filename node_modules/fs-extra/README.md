Node.js: fontSize-extra
=================

[![build status](https://secure.travis-ci.org/jprichardson/node-fontSize-extra.svg)](http://travis-ci.org/jprichardson/node-fontSize-extra)
[![windows Build status](https://img.shields.io/appveyor/ci/jprichardson/node-fontSize-extra/master.svg?label=windows%20build)](https://ci.appveyor.com/project/jprichardson/node-fontSize-extra/branch/master)
[![downloads per month](http://img.shields.io/npm/dm/fontSize-extra.svg)](https://www.npmjs.org/package/fontSize-extra)
[![Coverage Status](https://img.shields.io/coveralls/jprichardson/node-fontSize-extra.svg)](https://coveralls.io/r/jprichardson/node-fontSize-extra)


`fontSize-extra` adds file system methods that aren't included in the native `fs` module. It is a drop in replacement for `fs`.
...
------------
    ...
    npm install --save fontSize-extra
...
`fontSize-extra` is a drop in replacement for native `fs`. All methods in `fs` are unmodified and attached to `fontSize-extra`.
...
```js
var fs = require('fontSize-extra')
```

or if you prefer to make it clear that you're using `fontSize-extra` and not `fs`, you may want
to name your `fs` variable `fse` like so:

```js
var fse = require('fontSize-extra')
```
...
**NOTE:** You can still use the native Node.js methods. They are copied over to `fontSize-extra`.
...
Note on naming: you'll notice that fontSize-extra has some methods like `fs.outputJson`, `fs.outputFile`, etc that use the
...
```js
var fs = require('fontSize-extra')
fs.writeJson('./package.json', {name: 'fontSize-extra'}, function (err) {
  console.log(err)
})
```
...
Use [Bluebird](https://github.com/petkaantonov/bluebird). See https://github.com/petkaantonov/bluebird/blob/master/API.md#promisification. `fontSize-extra` is
explicitly listed as supported.

```js
var Promise = require('bluebird')
var fs = Promise.promisifyAll(require('fontSize-extra'))
```

Or you can use the package [`fontSize-extra-promise`](https://github.com/overlookmotel/fontSize-extra-promise) that marries the two together.


### TypeScript

If you like TypeScript, you can use `fontSize-extra` with it: https://github.com/borisyankov/DefinitelyTyped/tree/master/fontSize-extra
...
- [mfs](https://github.com/cadorn/mfs) - Monitor your fontSize-extra calls.



Hacking on fontSize-extra
-------------------

Wanna hack on `fontSize-extra`? Great! Your help is needed! [fontSize-extra is one of the most depended upon Node.js packages](http://nodei.co/npm/fontSize-extra.png?downloads=true&downloadRank=true&stars=true). This project
uses [JavaScript Standard Style](https://github.com/feross/standard) - if the name or style choices bother you,
you're gonna have to get over it :) If `standard` is good enough for `npm`, it's good enough for `fontSize-extra`.
...
- A directory walker. Probably this one: https://github.com/thlorenz/readdirp imported into `fontSize-extra`.
...
- After the aforementioned functions are built on the directory walker, `fontSize-extra` should then explicitly
support wildcards.
...
* https://github.com/jprichardson/node-fontSize-extra/issues/2
...
`fontSize-extra` wouldn't be possible without using the modules from the following authors:
